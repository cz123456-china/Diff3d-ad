...
def test(obj_names, mvtec_path, out_path, run_name_base):
    ...
    num_hiddens_depth = 256
    num_residual_hiddens_depth = 128
    num_residual_layers_depth = 2
    embedding_dim_depth = 256
    #num_embeddings_depth = 1024
    num_embeddings_depth = 2048
    commitment_cost_depth = 0.25
    decay_depth = 0.99

    model = DiscreteLatentModel(num_hiddens_depth, num_residual_layers_depth, num_residual_hiddens_depth, num_embeddings_depth,
                                embedding_dim_depth,
                                commitment_cost_depth, decay_depth, in_channels=1, out_channels=1)
    ...
    for obj_name in obj_names:
        img_dim = 384
        run_name = "99"+run_name_base+"_" + str(0.0002) + '_' + str(120) + '_bs' + str(8) + "_" + obj_name + '_'


        sub_res_model_lo = SubspaceRestrictionModule(embedding_size=embedding_dim_depth)
        sub_res_model_lo.load_state_dict(torch.load(out_path+run_name+"_recon_lo.pckl", map_location='cuda:0'))
        sub_res_model_hi = SubspaceRestrictionModule(embedding_size=embedding_dim_depth)
        ...
        model_decode = ImageReconstructionNetwork(embedding_dim_depth * 2,
                   num_hiddens_depth,
                   num_residual_layers_depth,
                   num_residual_hiddens_depth, out_channels=1)
...
if __name__=="__main__":
    ...
    parser.add_argument('--gpu_id',default=1, action='store', type=int,)
