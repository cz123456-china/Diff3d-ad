...
def train(training_dataset_loader, testing_dataset_loader, args, data_len,sub_class,class_type,device ):
    ...
    best_pixel_aupro=0.0
    best_epoch=0
    image_auroc_list=[]
    pixel_auroc_list=[]
    performance_x_list=[]
    filename = f"{sub_class}.txt"
    loss_file = open(filename, "w", buffering=1)
    filename1 = f"{sub_class}1.txt"
    loss_file1 = open(filename1, "w", buffering=1)
    ...
    for epoch in tqdm_epoch:
        ...
        for i, sample in enumerate(tbar):
            ...
            noise_loss, pred_x0,normal_t,x_normal_t,x_noiser_t = ddpm_sample.norm_guided_one_step_denoising(unet_model, aug_image,image, anomaly_label,args)
        ...
        if (epoch+1) % 10==0 :
            temp_image_auroc,temp_pixel_auroc,temp_pixel_aupro= eval(testing_dataset_loader,args,unet_model,seg_model,data_len,sub_class,device)
            image_auroc_list.append(temp_image_auroc)
            pixel_auroc_list.append(temp_pixel_auroc)
            performance_x_list.append(int(epoch))
            loss_file1.write(f"Epoch {epoch + 1} | Image AUROC: {temp_image_auroc} | Pixel AUROC: {temp_pixel_auroc} | Pixel AUPRO: {temp_pixel_aupro}\n")
            loss_file1.flush()  # 强制刷新缓冲区，确保数据写入磁盘
            if(temp_image_auroc+temp_pixel_auroc+temp_pixel_aupro>=best_image_auroc+best_pixel_auroc+best_pixel_aupro):
                if (temp_image_auroc>=best_image_auroc or True):
                    loss_file.write(f"Epoch {epoch + 1} | Image AUROC: {temp_image_auroc} | Pixel AUROC: {temp_pixel_auroc} | Pixel AUPRO: {temp_pixel_aupro}\n")
                    ...
                    best_pixel_aupro=temp_pixel_aupro
                    best_epoch = epoch
...
def eval(testing_dataset_loader,args,unet_model,seg_model,data_len,sub_class,device):
    ...
    return auroc_image,auroc_pixel,aupro_pixel
...
def main():
    ...
    mvtec_classes = ['cable_gland', 'bagel', 'carrot', 'cable_gland', 'dowel', 'foam', 'peach', 'cookie', 'rope', 'tire']
    ...
    for sub_class in current_classes:    
        ...
        training_dataset_loader = DataLoader(training_dataset, batch_size=2,shuffle=True,num_workers=8,pin_memory=True,drop_last=True)
